...
public class Personaje : MonoBehaviour 
{
	...
	[SerializeField] float fuerzaSalto = 0.5f;
	[Range(0, 1)]
	...
	public void Move(float move, bool crouch, bool jump){
		...
		if(aterrizado || airControl)
		{
			...
			// mueve al personaje
			GetComponent<Rigidbody>().velocity = new Vector2(move * velocidadMax, GetComponent<Rigidbody>().velocity.y);
        ...
        if ((aterrizado || !dobleSalto) && jump) {
            ...
            GetComponent<Rigidbody>().AddForce(new Vector2(0f, fuerzaSalto));
